        ...
        runnable = new Runnable() {
            ...
            public void run() {
                if (!callback.isStillInReels(packageName)) {
                    Log.d(TAG, "[REELS_STATE] Heartbeat: user no longer in Reels â€” stopping heartbeat");
                ...
                if (budgetState.isFreeBreakActive(now)) {
                    Log.d(TAG, "[FREE_BREAK] accumulateScrollBudget: skipping â€” free break active");
                } else {
                    if (budgetState.isWindowExpired(now)) {
                        Log.i(TAG, "[BUDGET] Scroll window expired â€” resetting");
