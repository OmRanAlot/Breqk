...
public class BudgetState {
    ...
    public boolean isExhausted(long now) {
        if (isFreeBreakActive(now)) {
            Log.d(TAG, "[BUDGET] [FREE_BREAK] isScrollBudgetExhausted: free break active â†’ returning false");
            return false;
        }
        ...
        if (isWindowExpired(now)) {
            Log.d(TAG, "[BUDGET] Window expired (windowStart=" + windowStartTime
                    ...
                    + " elapsed=" + (now - windowStartTime) + "ms) â†’ budget available (pending reset)");
            return false;
        }
