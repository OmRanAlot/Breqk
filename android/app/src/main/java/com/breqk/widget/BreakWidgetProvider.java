...
/**
 * BreakWidgetProvider — Home screen widget controller.
 *
 * PURPOSE:
 ...
 *   Tapping the app title opens the main Break app.
 *
 * HOW IT WORKS:
 *   1. Android calls onUpdate() when the widget is first placed or periodically refreshed.
 *   2. We inflate the layout (R.layout.widget_Break) via RemoteViews.
 *   3. We create PendingIntents for each clickable element:
 *      - Title → launches MainActivity (the main app)
 *      - Instagram icon → deep links to Break://browser/instagram
 *      - YouTube icon   → deep links to Break://browser/youtube
 ...
 *
 * RELATED FILES:
 *   - res/layout/widget_Break.xml            → Widget UI layout
 *   - res/xml/widget_Break_info.xml           → Widget metadata (size, update interval)
 ...
 * LOGGING:
 *   Tag: "BreakWidget"
 *   Filter with: adb logcat -s BreakWidget
 */
public class BreakWidgetProvider extends AppWidgetProvider {

    private static final String TAG = "BreakWidget";
     ...
     * and periodically based on updatePeriodMillis in widget_Break_info.xml (30 min).
     */
    @Override
    public void onUpdate(Context context, AppWidgetManager appWidgetManager, int[] appWidgetIds) {
        ...
        // Break://browser/instagram is caught by MainActivity's intent-filter in AndroidManifest.xml,
        // then React Navigation routes it to BrowserScreen with platform='instagram'.
        Intent instagramIntent = new Intent(Intent.ACTION_VIEW, Uri.parse("Break://browser/instagram"));
        ...
        // Break://browser/youtube — same flow as Instagram, routes to BrowserScreen with platform='youtube'.
        Intent youtubeIntent = new Intent(Intent.ACTION_VIEW, Uri.parse("Break://browser/youtube"));
        ...
        for (int appWidgetId : appWidgetIds) {
            ...
            RemoteViews views = new RemoteViews(context.getPackageName(), R.layout.widget_break);
