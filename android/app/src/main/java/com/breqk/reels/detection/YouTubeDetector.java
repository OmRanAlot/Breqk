 ...
 *
 * Extracted from ReelsInterventionService.isShortsLayout() (previously L1224–L1327),
 * hasShortsTextSignal() (L951–L995), and dumpYouTubeTreeIfNeeded() (L1852–L1902).
 ...
 *
 *   Tier 1 — Known container IDs (ShortFormIds.YOUTUBE_SHORTS_VIEW_IDS):
 *     Any matching node that passes isFullScreen() → confirmed Shorts.
 *
 *   Tier 2 — Secondary signals + no seekbar:
 *     ShortFormIds.YOUTUBE_SHORTS_SECONDARY_IDS + absence of YOUTUBE_SEEKBAR_ID.
...
public class YouTubeDetector implements ShortFormDetector {
     ...
     * Shorts-specific Activity or Fragment class (Tier 0 — O(1) detection).
     *
     ...
     * filtered out — they indicate YouTube's floating overlay, not real navigation.
     *
    ...
    public boolean checkClassNameTier0(String eventClass) {
        ...
        // Skip tree traversal entirely — getRootInActiveWindow() would return the overlay root.
    ...
    /**
     * Runs the Tier 1→Tier 2→Tier 3→Tier 4 ladder on the accessibility tree root.
     *
    ...
    public DetectResult detect(AccessibilityNodeInfo root) {
            ...
            if (nodes != null && !nodes.isEmpty()) {
                ...
                for (AccessibilityNodeInfo n : nodes) {
                    if (FullScreenCheck.isFullScreen(n, context, tag)) {
                        recycleAll(nodes);
                        Log.d(tag, "[TIER] TIER1 matched " + viewId + " (full-screen) → true");
        ...
        Log.d(tag, "[TIER] TIER1 — no known container IDs matched full-screen");
     ...
     * ShortFormIds.YOUTUBE_SHORTS_VIEW_IDS with the discovered IDs.
     *
     * @param root Root of the YouTube accessibility tree
     */
    public void dumpYouTubeTreeIfNeeded(AccessibilityNodeInfo root) {
        ...
        Log.w(tag, "YT_TREE_DUMP To fix: find the Shorts container ID below and add it to "
                + "ShortFormIds.YOUTUBE_SHORTS_VIEW_IDS in com.breqk.reels.ShortFormIds");
