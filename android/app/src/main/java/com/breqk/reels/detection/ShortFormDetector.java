 ...
 *
 * Implementations:
 *   - InstagramDetector — detects Instagram Reels via clips_viewer_* view IDs
 *   - YouTubeDetector   — detects YouTube Shorts via Tier 0-4 ladder
 *
...
public interface ShortFormDetector {
     ...
     *
     * Called from the accessibility thread — must be fast (no blocking I/O, no heavy
     * allocations). Tree traversal is acceptable; network access is not.
     *
     * Callers must NOT recycle {@code root} — the detector does not own the root node.
     ...
     *             — detector must handle null gracefully and return a not-detected result).
